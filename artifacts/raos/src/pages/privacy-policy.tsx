...
function renderMarkdown(text: string) {
  return text.split("\n").map((line, i) => {
    if (line.startsWith("# "))   return <h1 key={i} className="text-xl font-bold text-slate-900 mt-6 mb-2">{line.slice(2)}</h1>;
    if (line.startsWith("## "))  return <h2 key={i} className="text-base font-semibold text-slate-800 mt-5 mb-1.5 border-b border-slate-100 pb-1">{line.slice(3)}</h2>;
    if (line.startsWith("### ")) return <h3 key={i} className="text-sm font-semibold text-slate-700 mt-4 mb-1">{line.slice(4)}</h3>;
    if (line.startsWith("- "))   return <li key={i} className="ml-5 text-slate-600 text-sm leading-relaxed list-disc">{renderInline(line.slice(2))}</li>;
