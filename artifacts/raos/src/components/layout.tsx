...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = isApprentice
    ...
    : [
        ...
        ...(user?.role === "admin" ? [{ href: "/communications", label: "Communications", icon: Mail }] : []),
        ...
        ...(user?.role === "admin" ? [{ href: "/admin/training-registrations", label: "Training Registrations", icon: BookOpen }] : []),
      ];
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      {/* Sidebar */}
      <div className={cn(
        "no-print fixed inset-y-0 left-0 z-40 w-64 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:flex-shrink-0 flex flex-col",
