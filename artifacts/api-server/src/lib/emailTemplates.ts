export function buildTeacherEmail(studentName: string, link: string, debriefMeetingUrl?: string | null, debriefMeetingDate?: string | null, isPlaceholder?: boolean): string {
  ...
  const downloadButton = isPlaceholder
    ? `<div style="text-align:center;margin:28px 0">
        <span style="display:inline-block;background:#94a3b8;color:#fff;padding:12px 28px;border-radius:8px;font-weight:600;cursor:not-allowed;opacity:0.7">Download Report</span>
        <p style="margin:10px 0 0;font-size:12px;color:#ef4444;font-weight:600">⚠️ Placeholder — this button is inactive in the preview.<br/>A real, working link will be sent to the school when the report is officially released.</p>
      </div>`
