...
// ── Tutor App API ─────────────────────────────────────────────────────────────
// Secured by TUTOR_API_KEY env var (x-api-key header).
// These endpoints let an external tutor app create intake sessions and read back results
// without requiring session auth. Form submission uses the existing token-based flow.

function tutorApiKey(): string | null {
  return process.env.TUTOR_API_KEY ?? null;
}

function validateTutorApiKey(req: import("express").Request, res: import("express").Response): boolean {
  const expected = tutorApiKey();
  if (!expected) {
    res.status(503).json({ error: "not_configured", message: "TUTOR_API_KEY is not set on this server. Ask your administrator to configure it." });
    return false;
  }
  const provided = req.headers["x-api-key"];
  if (provided !== expected) {
    res.status(401).json({ error: "unauthorized", message: "Invalid API key. Pass your key as the x-api-key header." });
    return false;
  }
  return true;
}

function setCorsHeaders(req: import("express").Request, res: import("express").Response) {
...
// Preflight
router.options("/external/tutor/sessions", (req, res) => { setCorsHeaders(req, res); res.sendStatus(204); });
router.options("/external/tutor/sessions/:token/result", (req, res) => { setCorsHeaders(req, res); res.sendStatus(204); });

/**
 * POST /external/tutor/sessions
 * Create a new TLPI intake session for a student.
 *
 * Headers: x-api-key: <TUTOR_API_KEY>
 * Body: { studentName: string, tutorRef?: string, language?: "english" | "mandarin" | "cantonese" }
 *
 * Returns: { sessionToken, formUrl, submitUrl, studentName, form: { toolId, toolName, questions } }
 */
router.post("/external/tutor/sessions", async (req, res) => {
  setCorsHeaders(req, res);
  if (!validateTutorApiKey(req, res)) return;

  const { studentName, tutorRef, language } = req.body as {
    studentName?: string;
    tutorRef?: string;
    language?: string;
  };
  ...
  if (!studentName?.trim()) {
    res.status(400).json({ error: "bad_request", message: "studentName is required." });
    return;
  }
  ...
  const baseUrl = `${proto}://${host}`;
  const formUrl   = `${baseUrl}/external/${sessionToken}`;
  const submitUrl = `${baseUrl}/external/form/${sessionToken}/submit`;

  const lang = (["english", "mandarin", "cantonese"] as const).includes(language as "english")
    ? (language as "english" | "mandarin" | "cantonese")
    : "english";
  ...
  await db.insert(casesTable).values({
    ...
    dob: "not_provided",
    school: tutorRef?.trim() ?? "Independent Tutoring",
    languagePreference: lang,
    referralReason: "Tutoring intake",
    caseStatus: "active",
    currentPhase: "forms",
    progressPercentage: 0,
  });
  ...
  await db.insert(assignmentsTable).values({
    ...
    caseId,
    toolId: "TLPI",
    toolName: "Tutoring Learning Profile Inventory (TLPI)",
    respondentType: "self",
    respondentLabel: "Student",
    uniqueToken: sessionToken,
    uniqueLink: formUrl,
    qrCodeData: formUrl,
    status: "not_started",
    dueDate: null,
  ...
  const questions = await resolveQuestions("TLPI");
...
/**
 * GET /external/tutor/sessions/:token/result
 * Retrieve the submitted answers for a tutoring intake session.
 *
 * Headers: x-api-key: <TUTOR_API_KEY>
 * Returns: { sessionToken, studentName, status, submittedAt, answers }
 */
router.get("/external/tutor/sessions/:token/result", async (req, res) => {
  setCorsHeaders(req, res);
  if (!validateTutorApiKey(req, res)) return;
  ...
  const [caseRow] = await db
    .select({ studentName: casesTable.studentName, school: casesTable.school })
    .from(casesTable)
    .where(eq(casesTable.id, assignment.caseId))
    .limit(1);
  ...
  res.json({
    sessionToken: token,
    studentName: caseRow?.studentName ?? null,
    tutorRef: caseRow?.school ?? null,
