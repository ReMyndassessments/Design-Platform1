...
Return ONLY valid JSON (no markdown, no explanation):
{
  ...
  "productiveStruggle": "Clinical paragraph (150+ words) synthesising how ${c?.student_name?.split(" ")[0] ?? "the student"} responded when they encountered difficulty across the session. YOU MUST WRITE THIS SECTION — if no formal behavioral observation ratings are available, base the analysis entirely on the interview response data. Draw on: (1) any behavioral observation ratings/notes if present; (2) interview responses where the student said 'I don't know', gave no response, attempted and abandoned, or verbalised confusion; (3) answer status patterns (incorrect/no response clusters) that reveal where struggle occurred; (4) any self-correction, strategy switching, or help-seeking observed; (5) the contrast between domains where they responded confidently versus where they went silent or disengaged; and (6) what this pattern implies for how the student is likely to handle challenge in a classroom context. Quote the student directly and name specific samples. This section must never be left empty.",
