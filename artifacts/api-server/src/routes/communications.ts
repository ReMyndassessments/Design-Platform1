...
router.delete("/communications/campaigns/test-drafts", async (req, res) => {
  const deleted = await db.execute(sql`
    ...
    WHERE c.status = 'draft'
      ...
      AND NOT EXISTS (SELECT 1 FROM communication_recipients r WHERE r.campaign_id = c.id)
    RETURNING c.id
  `);
...
router.delete("/communications/campaigns/:id", async(req,res): Promise<void>=>{const r=await db.execute(sql`DELETE FROM communication_campaigns c WHERE c.id=${req.params.id} AND c.status IN ('draft','cancelled') AND NOT EXISTS (SELECT 1 FROM communication_recipients cr WHERE cr.campaign_id=c.id) RETURNING c.id`);if(!r.rows.length){res.status(409).json({error:"Only unsent campaigns with no recipient history can be permanently deleted"});return;}await writeAudit({eventType:"communications.campaign.deleted",actorId:req.userId,actorRole:req.userRole,metadata:{campaignId:req.params.id}});res.json({ok:true});});
