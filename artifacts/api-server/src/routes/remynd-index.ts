...
import { scoresTable, casesTable, assessmentToolsTable, assignmentsTable, responsesTable } from "@workspace/db/schema";
...
async function buildIndexData(caseId: string) {
  ...
  if (unscored.length > 0) {
    ...
    const responseRows = await db
      .select()
      .from(responsesTable)
      .where(inArray(responsesTable.assignmentId, unscoredIds));

    // Latest response per assignment (latest submitted_at wins)
    const latestResponse = new Map<string, typeof responseRows[0]>();
    for (const r of responseRows) {
      const existing = latestResponse.get(r.assignmentId);
      if (!existing || (r.submittedAt && (!existing.submittedAt || r.submittedAt > existing.submittedAt))) {
        latestResponse.set(r.assignmentId, r);
      }
    }
    ...
    for (const assignment of unscored) {
      ...
      if (Object.keys(rawDomainScores).length === 0) continue;

      const normalized = normalizeScores(rawDomainScores, scaleMax);
      const vals = Object.values(rawDomainScores);
