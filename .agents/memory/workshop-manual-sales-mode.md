...
The payment section is additive: paid workshops must retain the full school-information, interests, school-needs, future-interest, and consent intake.

**How to apply:** Keep the bypass limited to paid workshop public entry points and enforce it on the server as well as the UI. Never hide or drop existing intake fields when changing payment handling. QR receipt submission remains pending administrator verification; inquiry submission never creates paid access. Restore checkout only by disabling the workshop-specific flag after safely retesting Airwallex.