...
Paid workshop purchases use a server-controlled manual-sales mode: prospective attendees choose WeChat Pay, Alipay, or Credit Card and verify their email instead of creating an Airwallex payment intent. QR payments require an uploaded receipt; Credit Card triggers a ReMynd payment-link follow-up. Preserve the Airwallex integration for later restoration.
...
**How to apply:** Keep the bypass limited to paid workshop public entry points and enforce it on the server as well as the UI. Serve configured QR artwork through an API image endpoint rather than direct object paths. Never hide or drop shared intake fields on free or paid forms when changing payment handling. QR receipt submission remains pending administrator verification; inquiry submission never creates paid access. Restore checkout only by disabling the workshop-specific flag after safely retesting Airwallex.