...
**How to apply:** Keep the bypass limited to paid workshop public entry points and enforce it on the server as well as the UI. QR receipt submission remains pending administrator verification; inquiry submission never creates paid access. Restore checkout only by disabling the workshop-specific flag after safely retesting Airwallex.